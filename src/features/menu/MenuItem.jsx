...
function MenuItem({ pizza }) {
  ...
  return (
    <li className="flex gap-4 py-2">
      <img src={imageUrl} alt={name} className={`h-24 rounded-lg ${soldOut ? 'opacity-70 grayscale' : ''}`} />

      <div className="flex flex-grow flex-col pt-1">
        <p className="font-Baloo-Bhai-2-Swash font-medium">{name}</p>
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          {!soldOut && !isInCart && (
            <Button type="small" onClick={() => dispatch(addItem(newItem))}>
              Add to cart
            </Button>
          )}
        </div>
